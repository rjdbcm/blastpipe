...
class RESTRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_method(self, method):
        ...
        if route is None:
            self._extracted_from_handle_method_4(404, 'Route not found\n')
        elif method == 'HEAD':
            self._extracted_from_handle_method_9(route)
        elif 'file' in route:
            if method == 'GET':
                try:
                    ...
                    self._extracted_from_handle_method_4(404, 'File not found\n')
            else:
                self._extracted_from_handle_method_4(405, 'Only GET is supported\n')
        elif method in route:
            content = route[method](self)
            if content is None:
                self._extracted_from_handle_method_4(404, 'Not found\n')
            else:
                self._extracted_from_handle_method_9(route)
                if method != 'DELETE':
    ...
    # TODO Rename this here and in `handle_method`
    def _extracted_from_handle_method_9(self, route):
    ...
    # TODO Rename this here and in `handle_method`
    def _extracted_from_handle_method_4(self, arg0, arg1):
        self.send_response(arg0)
        self.end_headers()
        self.wfile.write(arg1.encode())
